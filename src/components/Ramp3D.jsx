...
export default function Ramp3D({ segments, largeur }) {
  ...
  useEffect(() => {
    ...
    segments.forEach(seg => {
      // Trapezoidal prism: shape in XY plane, extruded along +Z
      const shape = new THREE.Shape()
      shape.moveTo(seg.xStart, 0)
      shape.lineTo(seg.xEnd,   0)
      shape.lineTo(seg.xEnd,   seg.elevArr)
      shape.lineTo(seg.xStart, seg.elevDep)
      shape.closePath()

      const geom = new THREE.ExtrudeGeometry(shape, { depth: w, bevelEnabled: false })

      const mat = new THREE.MeshPhongMaterial({
        color:     SEG_COLOR[seg.type],
        shininess: 40,
      })
      ...
      const edgesMat = new THREE.LineBasicMaterial({
        color:       SEG_EDGE[seg.type],
        transparent: true,
        opacity:     0.55,
      })
      scene.add(new THREE.LineSegments(new THREE.EdgesGeometry(geom, 10), edgesMat))
    })

    // ── Ground ─────────────────────────────────────────────────────────────
    const groundMat  = new THREE.MeshLambertMaterial({ color: 0xdde3ec })
    const groundGeom = new THREE.PlaneGeometry(totalW + 12, w + 12)
    const ground = new THREE.Mesh(groundGeom, groundMat)
    ground.rotation.x = -Math.PI / 2
    ground.position.set(totalW / 2, -0.01, w / 2)
    ground.receiveShadow = true
    scene.add(ground)

    const gridSize = Math.ceil(Math.max(totalW + 10, w + 10))
